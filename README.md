...
- **Animated character** — a rigged low-poly figure (generated by
  `assets/models/generated_human/`) with four baked clips — idle, walk, jump and
  crouch — that the player controller crossfades from live motion state; the
  figure's feet are measured and planted on the ground automatically.
- **Motion input pipeline** — webcam → Python MediaPipe pose → UDP → in-game
  character movement (marching, leaning to turn, jumping, crouching).
